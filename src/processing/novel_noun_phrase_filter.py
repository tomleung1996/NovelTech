...
class NovelNounPhraseFilter:
    def __init__(self, config_path='config/config.yaml'):
        with open(config_path, mode='r', encoding='utf-8') as f:
            ...
            self.existing_noun_phrase_set_path = config['output_paths']['existing_noun_phrase_set']
            self.current_noun_phrases_path = config['output_paths']['current_noun_phrases']
            self.output_path = config['output_paths']['current_novel_noun_phrases']

            logger.info('正在统计现存名词短语……')
            self.existing_noun_phrase_set = pickle.load(open(self.existing_noun_phrase_set_path, mode='rb'))
    ...
    def filter_novel_noun_phrases(self):
        ...
        for uuid, np_list in tqdm(self.current_noun_phrase_dict.items()):
            new_np_list = []
            for np in np_list:
                if np not in self.existing_noun_phrase_set:
                    new_np_list.append(np)
            self.current_noun_phrase_dict[uuid] = new_np_list
        logger.info('过滤完成')
